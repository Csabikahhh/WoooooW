...
class Shape {
  ...
  init() {
    this.criterion = canvas.width > canvas.height ? canvas.width * 1.1 : canvas.height;
    this.num = 3;
    this.lattice = null;
  ...
  makeSqLattice() {
    ...
    for (let i = 0; i <= this.num; i++) {
      for (let j = 0; j <= m; j++) {
        ...
        let nv3 = nv.add(nv2);
        this.lattice[i][j] = new Vector2d(nv3.vx, nv3.vy % (this.criterion + this.scalar));
      }
    }
  }
  ...
  makeRecurTriangle() {
    ...
    for (let i = 0; i < 2; i++) {
      v[i] = new Vector2d(Math.cos(i * Math.PI / 3), Math.sin(i * Math.PI / 3));
      v[i].scale(this.scalar / Math.sqrt(3));
    }
    for (let i = 0; i < 36; i++) {
      ctx.strokeStyle = 'hsl(' + i * Math.sin(Date.now() / 1000) * 10 + ', 80%, 60%)';
      ctx.beginPath();
      for (let i = 0; i < 3; i++) {
        if (i === 0) {
  ...
  render() {
    this.drawTiling();
    this.gap = Math.cos(Date.now() / 5000) < 0 ? -Math.cos(Date.now() / 5000) : Math.cos(Date.now() / 5000);
  }
}
